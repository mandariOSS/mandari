...
class FactionMeetingListView(WorkViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["stats"] = {
            ...
            "pending_protocol": all_meetings.filter(
                status="completed",
...
class FactionMeetingCreateView(WorkViewMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            previous = FactionMeeting.find_previous_meeting(
                self.organization,
                before_date=meeting.start
            )
...
class FactionMeetingDetailView(WorkViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        agenda_items = meeting.agenda_items.filter(
            ...
            "related_agenda_item", "approves_meeting"
        ).prefetch_related(
            "protocol_entries",
        ...
        existing_member_ids = meeting.attendances.filter(
            membership__isnull=False
        ).values_list("membership_id", flat=True)

        from apps.tenants.models import Membership
        context["available_members"] = Membership.objects.filter(
            organization=self.organization,
            is_active=True
        ).exclude(
...
class FactionProtocolView(WorkViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Attendees present
        context["present_members"] = meeting.attendances.filter(
            status="present"
        ).select_related("membership__user")

        # All members for speaker selection
        context["all_members"] = self.organization.memberships.filter(
            is_active=True
        ).select_related("user")
        ...
        # Can edit protocol
        context["can_edit"] = (
            meeting.status in ["ongoing", "completed"] and
            not meeting.protocol_approved
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "add":
            ...
            if parent:
                ...
                existing_items = meeting.agenda_items.filter(
                    visibility=visibility,
                    parent__isnull=True
            ...
            for idx, item_id in enumerate(order):
                FactionAgendaItem.objects.filter(
                    id=item_id,
                    meeting=meeting
                ).update(order=idx)
            ...
            if item and direction in ("up", "down"):
                ...
                siblings = list(meeting.agenda_items.filter(
                    visibility=item.visibility,
                    parent__isnull=True,
                    is_approval_item=False
                ).order_by("order"))
    ...
    def post(self, request, *args, **kwargs):
        ...
        if meeting.invitation_sent:
            ...
            if sent_count > 0:
                ...
                messages.warning(request, "Einladungsstatus aktualisiert. Keine E-Mails versendet (keine E-Mail-Adressen hinterlegt).")
...
class FactionAttendanceStatusView(WorkViewMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        valid_statuses = ["invited", "confirmed", "declined", "tentative", "present", "absent", "excused"]
        ...
        if new_status == "present" and not attendance.checked_in_at:
            attendance.checked_in_at = timezone.now()
        elif new_status in ("absent", "excused", "declined") and attendance.checked_in_at and not attendance.checked_out_at:
            attendance.checked_out_at = timezone.now()
