...
def oparl_datetime(dt: Optional[datetime]) -> Optional[str]:
...
class OParlMeetingListView(OParlMixin, View):
    ...
    def get(self, request, tenant_slug: str):
        ...
        meetings = SessionMeeting.objects.filter(
            tenant=tenant,
            is_public=True,
        ).select_related("organization").order_by("-start")[:100]
        ...
        return self.oparl_json_response({
            "data": data,
            "pagination": {
                "totalElements": SessionMeeting.objects.filter(
...
class OParlPaperListView(OParlMixin, View):
    ...
    def get(self, request, tenant_slug: str):
        ...
        return self.oparl_json_response({
            "data": data,
            "pagination": {
                "totalElements": SessionPaper.objects.filter(
                    tenant=tenant, is_public=True
                ).count(),
                "elementsPerPage": 100,
                "currentPage": 1,
        ...
        for meeting in meetings:
            ...
            if session_user and self.check_permission(session_user, "view_non_public_meetings"):
                if not meeting.is_public:
                    item["internal_notes"] = meeting.get_internal_notes_decrypted() if hasattr(meeting, "get_internal_notes_decrypted") else None
    ...
    def get(self, request, tenant_slug: str):
        ...
        for paper in papers:
            item = {
                ...
                } if paper.main_organization else None,
            }
...
class SessionApplicationListAPIView(SessionAPIMixin, View):
    ...
    def get(self, request, tenant_slug: str):
        ...
        applications = SessionApplication.objects.filter(
            tenant=tenant
        ).select_related(
            "submitting_organization", "target_organization"
        ...
        for app in applications:
            data.append({
                ...
                } if app.submitting_organization else None,
                "target_organization": {
                    "id": str(app.target_organization_id),
                    "name": app.target_organization.name,
                } if app.target_organization else None,
...
class ApplicationSubmitAPIView(SessionAPIMixin, View):
    ...
    def _authenticate_token(self, request, tenant: SessionTenant) -> Optional[SessionAPIToken]:
    ...
    def post(self, request, tenant_slug: str):
        ...
        if data.get("submitting_organization_id"):
            try:
                ...
                submitting_org = Organization.objects.get(
                    id=data["submitting_organization_id"]
                )
