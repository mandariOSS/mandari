...
def markdownify(text):
    ...
    html = html.replace('<li>', '<li class="flex items-start gap-3 text-gray-600 dark:text-gray-300"><span class="w-2 h-2 bg-primary-500 rounded-full mt-2 flex-shrink-0"></span><span>')
    html = html.replace('</li>', '</span></li>')

    # Tables
    html = html.replace('<table>', '<div class="overflow-x-auto my-8"><table class="min-w-full border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">')
    ...
    html = html.replace('<th>', '<th class="px-4 py-3 text-left text-sm font-semibold text-gray-900 dark:text-white">')
    html = html.replace('<td>', '<td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">')

    # Code
    html = html.replace('<code>', '<code class="bg-gray-100 dark:bg-gray-800 text-primary-600 dark:text-primary-400 px-1.5 py-0.5 rounded text-sm font-mono">')
    html = html.replace('<pre>', '<pre class="bg-gray-900 text-gray-100 rounded-xl p-4 overflow-x-auto my-6">')

    # Blockquotes
    html = html.replace('<blockquote>', '<blockquote class="border-l-4 border-primary-500 bg-primary-50 dark:bg-primary-900/20 pl-4 pr-4 py-3 my-6 text-gray-700 dark:text-gray-300 rounded-r-lg">')
    ...
    # Links
    html = html.replace('<a ', '<a class="text-primary-600 dark:text-primary-400 hover:underline font-medium" target="_blank" rel="noopener noreferrer" ')
