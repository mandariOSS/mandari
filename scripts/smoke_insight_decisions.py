...
sub.refresh_from_db()
check("Link aus der Mail zeigt Bestätigungsseite, ändert nichts", resp.status_code == 200 and not sub.confirmed)
resp = anon.post(f"/insight/beschluesse/abo/bestaetigen/{sub.token}/")
sub.refresh_from_db()
...
sub.refresh_from_db()
check("Abmeldelink zeigt Bestätigungsseite, ändert nichts", resp.status_code == 200 and sub.unsubscribed_at is None)
resp = anon.post(f"/insight/beschluesse/abo/abmelden/{sub.token}/")
sub.refresh_from_db()
